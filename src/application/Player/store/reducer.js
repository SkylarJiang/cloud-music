...
const defaultState = fromJS({
    ...
    sequencePlayList: [], //顺序列表（因为之后会有随机模式，列表回乱序，因此那这个保存顺序）
    playList:[],
    mode: playMode.sequence, //播放模式
    currentIndex: -1, //当前歌曲在播放列表的索引位置
    showPlayList: false, //是否展示播放列表（就是点击那个键会展示播放列表吧）
    currentSong: {} //当前歌曲的信息
});

const handleDeleteSong = (state, song) => {
  //实现一个深拷贝，也可以使用 loadsh 库的 deepClone 方法
  ...
  //如果删除的歌曲排在当前歌曲的前面，那么currentIndex--才能让当前歌曲正常播放
  if (fpIndex < currentIndex) currentIndex--;
  
  // 在顺序列表sequenceList中直接删除就可以了
