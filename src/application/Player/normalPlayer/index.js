...
function NormalPlayer(props) {
    ...
    const normalPlayRef = useRef();
    const cdWrapperRef = useRef();

    const transform = prefixStyle('transform')

    const enter = () => {
        normalPlayRef.current.style.display = 'block';
    ...
    const afterLeave = () => {
        ...
        // 现在要把 normalPlayer 这个DOM 隐藏起来
        // 因为 CSSTransition 的工作只是把动画执行一遍
        // 不设置为 none 是因为现在全屏播放器页面还是存在
        normalPlayRef.current.style.display = 'none';
    };
    ...
    return (
        <CSSTransition
            ...
            onExited={afterLeave}
        >
            <NormalPlayerContainer ref={normalPlayRef}>
